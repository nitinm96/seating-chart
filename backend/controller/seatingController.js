...
const getAllGuests = (req, res) => {
  ...
  db.query(query, (err, result) => {
    ...
    if (result.length === 0) {
      console.log({ message: "no guests found in database" });
      return res
        .status(200)
        .json({
          error: "No guests found in database",
    ...
    return res
      .status(200)
      .json({ allGuests: result, guestCount: result.length });
  });
};
...
const getGuest = (req, res) => {
  ...
  //validate id
  if (!id || isNaN(id)) {
    return res.status(400).json({ error: "invalid id from params." });
  }
  ...
  db.query(query, [id], (err, result) => {
    ...
    return res
      .status(200)
      .json({ message: "Guest found.", foundGuest: result });
  });
};
...
const addGuest = (req, res) => {
  ...
  db.query(queryFind, [fullName], (err, findResult) => {
    ...
    if (findResult.length != 0) {
      ...
      return res
        .status(409)
        .json({ error: "Guest already exists", guest: findResult });
    }

    //query to insert new guest
    const query =
      "INSERT INTO guests (`guest_name`,`table_number`) VALUES (?,?)";
    db.query(query, [fullName, tableNumber], (err, result) => {
      if (err) {
        console.error({ error: `failed to insert guest: ${err} ` });
        return res
          .status(500)
          .json({ error: "error inserting guest into database." });
      }
      console.log({
        message: `Guest ${fullName} added at table number ${tableNumber}.`,
      });
...
const updateGuest = (req, res) => {
  ...
  if (!fullName || !tableNumber || isNaN(tableNumber)) {
    return res.status(400).json({ error: "invalid name or table number." });
  }
  ...
  db.query(queryFind, [id], (err, result) => {
    ...
    if (result.length === 0) {
      console.error({
        error: `Failed to update, guest not found at id: ${id}`,
      });
      return res.status(404).json({ error: "Guest not found." });
    }
    //sql query for updating guest
    const queryGuest =
      "UPDATE guests SET `guest_name` = (?), `table_number` = (?) WHERE `guest_id` = (?)";
...
const deleteGuest = (req, res) => {
  ...
  db.query(queryFind, [id], (err, resultGuest) => {
    ...
    if (resultGuest.length === 0) {
      console.error({
        error: `Failed to delete, guest not found at id: ${id}`,
      });
      return res.status(404).json({ error: "Guest not found." });
    }
    ...
    db.query(queryDelete, [id], (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ message: "server error deleting guest." });
      }
      console.log({ message: `guest with id: ${id} deleted.` });
      return res
        .status(200)
        .json({ message: "Guest Deleted.", deletedGuest: resultGuest });
    });
  });
};
